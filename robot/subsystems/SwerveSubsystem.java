...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public void updateVision() {
    ...
    Optional<Translation3d> gyroAccel = swerveDrive.getGyro().getAccel();
    if (gyroAccel.isPresent()) {
      // if our angular velocity is greater than 720 degrees persecond, ignore vision
      // updates
      // if we see no aprilTags
      LimelightHelpers.PoseEstimate megaTag2 = LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2("limelight");
      if (!(Math.abs(gyroAccel.get().getZ()) > 720) || !(megaTag2.tagCount == 0)) {
        swerveDrive.addVisionMeasurement(megaTag2.pose, megaTag2.timestampSeconds, VecBuilder.fill(.7, .7, 9999999));
        return;
      }
    } else { // use megatag1 if megatag2 requirments are not met
      boolean doRejectUpdate = false;
      LimelightHelpers.PoseEstimate megatag1 = LimelightHelpers.getBotPoseEstimate_wpiBlue("limelight");
